...
public class PersonClientController {
    ...
    public void testUpload() {
        Resource resource = new FileSystemResource("/Users/lizhenghui/Public/test.zip");
        String fileName = "pshdhx" + UUID.randomUUID();
        String fileExt = resource.getFilename().substring(resource.getFilename().lastIndexOf(".") + 1);
        Flux<DataBuffer> resourceFlux = DataBufferUtils.read(resource, new DefaultDataBufferFactory(), 1024 * 1024 * 8)
        ...
        Flux<UploadStatus> uploadStatusFlux = this.requesterMonos
                ...
                .doOnNext(o -> System.out.println("上传进度:" + o)).doOnComplete(() -> {
                    try {
                        resource.readableChannel().close();
                    } catch (IOException e) {
